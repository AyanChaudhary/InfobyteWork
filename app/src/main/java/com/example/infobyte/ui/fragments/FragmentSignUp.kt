...
import com.example.infobyte.databinding.FragmentSignupBinding
...
class FragmentSignUp: Fragment(R.layout.fragment_signup) {
    ...
    private fun signUp() {
        ...
        if(email.isNotEmpty() && password.isNotEmpty()){
            val user=auth.createUserWithEmailAndPassword(email,password).addOnCompleteListener {
                if(it.isSuccessful){
                    findNavController().navigate(R.id.action_fragmentSignUp_to_fragmentSignInWithGoogle)
                }
                else{
                    Snackbar.make(requireView(),"SignUp failed : ${it.toString()}",Snackbar.LENGTH_SHORT).show()
                }
            }
        }
    }
